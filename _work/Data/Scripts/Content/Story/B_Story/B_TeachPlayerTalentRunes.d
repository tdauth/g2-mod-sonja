...
func int B_TeachPlayerTalentRunes (var C_NPC slf, var C_NPC oth, var int spell)
{
	...
	// Sonja
	if (spell == SPL_TrfSheep)				{	PLAYER_TALENT_RUNES[SPL_TrfSheep]				= TRUE; CreateInvItems (ScrollTrader, ItSc_TrfSheep, 1);
	B_LogEntry (TOPIC_TalentRunes,"Zutaten für die Rune ´VERWANDLUNG SCHAF´: 1 Schafsfell");	};
	if (spell == SPL_TrfScavenger)				{	PLAYER_TALENT_RUNES[SPL_TrfScavenger]				= TRUE; CreateInvItems (ScrollTrader, ItSc_TrfScavenger, 1);
	B_LogEntry (TOPIC_TalentRunes,"Zutaten für die Rune ´VERWANDLUNG SCAVENGER: 10 Rohes Fleisch");	};
	if (spell == SPL_TrfGiantRat)				{	PLAYER_TALENT_RUNES[SPL_TrfGiantRat]				= TRUE; CreateInvItems (ScrollTrader, ItSc_TrfGiantRat, 1);
	B_LogEntry (TOPIC_TalentRunes,"Zutaten für die Rune ´VERWANDLUNG RIESENRATTE: 10 Rohes Fleisch");	};
	if (spell == SPL_TrfGiantBug)				{	PLAYER_TALENT_RUNES[SPL_TrfGiantBug]				= TRUE; CreateInvItems (ScrollTrader, ItSc_TrfGiantBug, 1);
	B_LogEntry (TOPIC_TalentRunes,"Zutaten für die Rune ´VERWANDLUNG FELDRÄUBER: 2 Feldräuberzangen");	};
