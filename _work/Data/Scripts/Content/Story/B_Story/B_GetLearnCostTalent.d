...
func int B_GetLearnCostTalent (var C_NPC oth, var int talent, var int skill)
{
	...
	if (talent == NPC_TALENT_RUNES)
	{
		...
		// Sonja
		else if (skill == SPL_TrfSheep		    ) 	{	kosten = (2);	}
		else if (skill == SPL_TrfScavenger		) 	{	kosten = (3);	}
		else if (skill == SPL_TrfGiantRat		) 	{	kosten = (3);	}
		else if (skill == SPL_TrfGiantBug		) 	{	kosten = (3);	}

		else 										{	kosten = (5);	};
	};

	// Sonja
	if (talent == NPC_TALENT_WOMANIZER)
	{
        kosten = skill;
    };

    if (talent == NPC_TALENT_PIMP)
    {
        kosten = skill;
    };
