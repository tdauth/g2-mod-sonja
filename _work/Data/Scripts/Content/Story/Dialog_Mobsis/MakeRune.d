...
//*******************************************************
INSTANCE PC_Circle_Transform (C_Info)
{
	npc				= PC_Hero;
	condition		= PC_Circle_Transform_Condition;
	information		= PC_Circle_Transform_Info;
	permanent		= TRUE;
	description		= "Runen Verwandlung erschaffen";
};
FUNC INT PC_Circle_Transform_Condition ()
{
	if( (PLAYER_MOBSI_PRODUCTION	==	MOBSI_MAKERUNE)
	&& ( (PLAYER_TALENT_RUNES[SPL_TrfSheep] == TRUE) || (PLAYER_TALENT_RUNES[SPL_TrfScavenger] == TRUE) || (PLAYER_TALENT_RUNES[SPL_TrfGiantRat] == TRUE) || (PLAYER_TALENT_RUNES[SPL_TrfGiantBug] == TRUE)))
	{
		return TRUE;
	};
};
FUNC VOID PC_Circle_Transform_Info()
{
	Info_ClearChoices (PC_Circle_Transform);

	if (PLAYER_TALENT_RUNES[SPL_TrfGiantBug] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Transform,NAME_SPL_TrfGiantBug,PC_ItRu_TrfGiantBug_Info);
	};

	if (PLAYER_TALENT_RUNES[SPL_TrfGiantRat] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Transform,NAME_SPL_TrfGiantRat,PC_ItRu_TrfGiantRat_Info);
	};

	if (PLAYER_TALENT_RUNES[SPL_TrfScavenger] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Transform,NAME_SPL_TrfScavenger,PC_ItRu_TrfScavenger_Info);
	};

	if (PLAYER_TALENT_RUNES[SPL_TrfSheep] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Transform,NAME_SPL_TrfSheep,PC_ItRu_TrfSheep_Info);
	};

	Info_AddChoice 	  (PC_Circle_Transform,DIALOG_BACK,PC_Circle_Transform_BACK);
};
FUNC VOID PC_Circle_Transform_BACK()
{
	Info_ClearChoices (PC_Circle_Transform);
};

FUNC VOID PC_ItRu_TrfSheep_Info ()
{
	if (Npc_HasItems (hero, ItSc_TrfSheep) 	 >= 1)
...
FUNC VOID PC_ItRu_TrfScavenger_Info ()
{
	if (Npc_HasItems (hero, ItSc_TrfScavenger) 	 >= 1)
	&& (Npc_HasItems (hero, ItFoMuttonRaw) >= 10)
	{
		Npc_RemoveInvItems  (hero,ItSc_TrfScavenger, 	  1 );
		Npc_RemoveInvItems  (hero,ItFoMuttonRaw,10	);

		CreateInvItems 	   (hero,ItRu_TrfScavenger,1);
		Print (PRINT_RunESuccess);
	}
	else
	{
...
FUNC VOID PC_ItRu_TrfGiantRat_Info ()
{
	if (Npc_HasItems (hero, ItSc_TrfGiantRat) 	 >= 1)
	&& (Npc_HasItems (hero, ItFoMuttonRaw) >= 10)
	{
		Npc_RemoveInvItems  (hero,ItSc_TrfGiantRat, 	  1 );
		Npc_RemoveInvItems  (hero,ItFoMuttonRaw,10	);

		CreateInvItems 	   (hero,ItRu_TrfGiantRat,1);
		Print (PRINT_RunESuccess);
	}
	else
	{
...
FUNC VOID PC_ItRu_TrfGiantBug_Info ()
{
	if (Npc_HasItems (hero, ItSc_TrfGiantBug) 	 >= 1)
	&& (Npc_HasItems (hero, ItAt_BugMandibles) >= 2)
	{
		Npc_RemoveInvItems  (hero,ItSc_TrfGiantBug, 	  1 );
		Npc_RemoveInvItems  (hero,ItAt_BugMandibles,2	);

		CreateInvItems 	   (hero,ItRu_TrfGiantBug,1);
		Print (PRINT_RunESuccess);
	}
	else
	{
...
//*******************************************************
INSTANCE PC_Circle_Dragon (C_Info)
{
	npc				= PC_Hero;
	condition		= PC_Circle_Dragon_Condition;
	information		= PC_Circle_Dragon_Info;
	permanent		= TRUE;
	description		= "Runen Drachen erschaffen";
};
FUNC INT PC_Circle_Dragon_Condition ()
{
	if( (PLAYER_MOBSI_PRODUCTION	==	MOBSI_MAKERUNE)
	&& ( (PLAYER_TALENT_RUNES[SPL_TrfDragon] == TRUE) || (PLAYER_TALENT_RUNES[SPL_SummonDragon] == TRUE)))
	{
		return TRUE;
	};
};
FUNC VOID PC_Circle_Dragon_Info()
{
	Info_ClearChoices (PC_Circle_Dragon);

	if (PLAYER_TALENT_RUNES[SPL_SummonDragon] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Dragon,NAME_SPL_SummonDragon,PC_ItRu_SummonDragon_Info);
	};

	if (PLAYER_TALENT_RUNES[SPL_TrfDragon] == TRUE)
	{
		Info_AddChoice 	  (PC_Circle_Dragon,NAME_SPL_TrfDragon,PC_ItRu_TrfDragon_Info);
	};

	Info_AddChoice 	  (PC_Circle_Dragon,DIALOG_BACK,PC_Circle_Dragon_BACK);
};
FUNC VOID PC_Circle_Dragon_BACK()
{
	Info_ClearChoices (PC_Circle_Dragon);
};

FUNC VOID PC_ItRu_TrfDragon_Info ()
{
	if (Npc_HasItems (hero, ItSc_TrfDragon) 	 >= 1)
	&& (Npc_HasItems (hero, ItAt_DragonScale) >= 1)
	{
		Npc_RemoveInvItems  (hero,ItSc_TrfDragon, 	  1 );
		Npc_RemoveInvItems  (hero,ItAt_DragonScale,1	);

		CreateInvItems 	   (hero,ItRu_TrfDragon,1);
		Print (PRINT_RunESuccess);
	}
	else
	{
...
FUNC VOID PC_ItRu_SummonDragon_Info ()
{
	if (Npc_HasItems (hero, ItSc_SummonDragon) 	 >= 1)
	&& (Npc_HasItems (hero, ItAt_DragonScale) >= 1)
	{
		Npc_RemoveInvItems  (hero,ItSc_SummonDragon, 	  1 );
		Npc_RemoveInvItems  (hero,ItAt_DragonScale,1	);

		CreateInvItems 	   (hero,ItRu_SummonDragon,1);
		Print (PRINT_RunESuccess);
	}
	else
	{
