...
INSTANCE DIA_BAU_16_JOIN(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Ich will mehr ¸ber die Sˆldner wissen!";
};                       
...
{	
	AI_Output (other, self, "DIA_BAU_16_JOIN_15_00"); //Ich will mehr ¸ber die Sˆldner wissen!
...
{	
	...
	AI_Output (self, other, "DIA_BAU_16_PEOPLE_16_01"); //Wenn du mich fragst, sind das die Sˆldner.
...
{	
	...
	AI_Output (self, other, "DIA_BAU_16_LOCATION_16_01"); //Es gibt drei Hˆfe hier. Onars im Osten und Sekobs im Norden das Tals.
...
{	
	...
	if (Kapitel == 1)
	{
		AI_Output (self,other,"DIA_BAU_16_STANDARD_16_01"); //Die Truppen aus der Stadt holen uns nicht mehr Vieh und Ernte weg! Wir spielen das Spiel nicht mehr mit, wir kˆnnen uns jetzt selbst verteidigen!
	};

	if (Kapitel == 2)
	{
		AI_Output (self,other,"DIA_BAU_16_STANDARD_16_02"); //Es heiﬂt, es w¸rde sich eine Armee im Minental sammeln. Bald werden sie hier sein.
	};

	if (Kapitel == 3)
	{
		AI_Output (self,other,"DIA_BAU_16_STANDARD_16_03"); //Ich habe gehˆrt, dass es im Minental Drachen geben soll. Kaum zu glauben.
	};

	if (Kapitel == 4)
	{
		AI_Output (self,other,"DIA_BAU_16_STANDARD_16_04"); //Einige der Sˆldner sind abgezogen. Ich weiﬂ nicht, ob das gut oder schlecht ist.
	};
...
// 									AUFREISSEN
// *************************************************************************
INSTANCE DIA_BAU_16_AUFREISSEN(C_INFO)
{
	nr			= 90;
	condition	= DIA_BAU_16_AUFREISSEN_Condition;
	information	= DIA_BAU_16_AUFREISSEN_Info;
	permanent	= TRUE;
	description = "(Aufreiﬂen)";
};
func INT DIA_BAU_16_AUFREISSEN_Condition()
{
	return TRUE;
};

FUNC VOID DIA_BAU_16_AUFREISSEN_Info()
{
    B_Aufreissen();
};
...
FUNC VOID B_AssignAmbientInfos_BAU_16 (var c_NPC slf)
{
	...
	DIA_BAU_16_AUFREISSEN.npc			= Hlp_GetInstanceID(slf);
};
