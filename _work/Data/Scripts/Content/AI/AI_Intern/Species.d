...
// Steinwächter
	...
	// Sonja
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // Beschworener DRACHE (Feuerdrache / Eisdrache / Felsdrache / Sumpfdrache / Untoter Drache)

	SWIM_TIME				[GIL_SUMMONED_DRAGON] = SWIM_TIME[GIL_DRAGON];
	DIVE_TIME		 		[GIL_SUMMONED_DRAGON] = DIVE_TIME[GIL_DRAGON];
	WATER_DEPTH_KNEE 		[GIL_SUMMONED_DRAGON] = WATER_DEPTH_KNEE[GIL_DRAGON];
	WATER_DEPTH_CHEST		[GIL_SUMMONED_DRAGON] = WATER_DEPTH_CHEST[GIL_DRAGON];

	STEP_HEIGHT		 		[GIL_SUMMONED_DRAGON] = STEP_HEIGHT[GIL_DRAGON]; //sonst "fällt" der Drache in manchen Drehungsframes (geht in FallDown)
	SLIDE_ANGLE		 		[GIL_SUMMONED_DRAGON] = SLIDE_ANGLE[GIL_DRAGON];

	DISABLE_AUTOROLL 		[GIL_SUMMONED_DRAGON] = DISABLE_AUTOROLL[GIL_DRAGON];
	TURN_SPEED				[GIL_SUMMONED_DRAGON] = TURN_SPEED[GIL_DRAGON];

	// Fallhöhe / Fallschaden
	FALLDOWN_HEIGHT			[GIL_SUMMONED_DRAGON] = FALLDOWN_HEIGHT[GIL_DRAGON];
	FALLDOWN_DAMAGE			[GIL_SUMMONED_DRAGON] = FALLDOWN_DAMAGE[GIL_DRAGON];

	// Kampf
	FIGHT_RANGE_BASE		[GIL_SUMMONED_DRAGON] = FIGHT_RANGE_BASE[GIL_DRAGON];
	FIGHT_RANGE_FIST		[GIL_SUMMONED_DRAGON] = FIGHT_RANGE_FIST[GIL_DRAGON]; //W = Base + Fist = 600 //Schiebefaktor NICHT 450 (*0.75) sondern Kürzer!!!
	FIGHT_RANGE_G			[GIL_SUMMONED_DRAGON] = FIGHT_RANGE_G[GIL_DRAGON]; //G = W + G = 900
